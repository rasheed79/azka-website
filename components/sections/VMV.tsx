...
export default function VMV() {
  ...
  return (
    <section className="py-16 lg:py-28 relative bg-gradient-to-b from-transparent via-slate-900/20 to-transparent">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <AnimatedSection delay={0.15} className="flex justify-center mb-8 sm:mb-12">
          <div className="inline-flex items-center p-1 rounded-2xl bg-slate-800/60 border border-slate-700/50 w-full sm:w-auto overflow-x-auto">
            {tabs.map(({ id, label, icon: Icon }) => (
              ...
              >
                ...
                <span className={`relative z-10 flex items-center gap-2 ${active === id ? 'text-white' : 'text-slate-400 hover:text-slate-200'}`}>
                  <Icon size={15} />
                  {label}
                </span>
              </button>
              ...
              <StaggerContainer staggerDelay={0.08} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {valuesRaw.map((value) => (
                  <StaggerItem key={value.title}>
                    <div className="p-6 rounded-2xl bg-slate-800/50 border border-slate-700/50 hover:border-green-500/30 hover:bg-slate-800/80 transition-all duration-200 group">
                      ...
                      <h4 className="text-white font-semibold mb-2 group-hover:text-green-300 transition-colors">{value.title}</h4>
